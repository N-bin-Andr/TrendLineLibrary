...
Подолжение разработки:
📋 Резюме для нового диалога: Критические уроки и найденные решения
🧩 Контекст проекта
Разрабатываем TrendLineLibrary для Tiger Trade Platform на C#.

Основная стабильная версия — в ветке main на GitHub.
Работаем над созданием горизонтального отрезка (инструмент, которого нет в платформе).

🔍 Найденные проблемы и их решения

1. Платформа создаёт 3 контрольные точки вместо 2
Проблема: Tiger Trade использует 3 точки для своих объектов (вероятно, для Безье). Наш объект ожидает 2 точки.

Решение:
НЕ изменять массив ControlPoints.
В Prepare() брать первые две точки для отрисовки:

csharp
Point p1 = ToPoint(ControlPoints[0]);
Point p2 = ToPoint(ControlPoints[1]);
Сохранять их в _startScreen и_endScreen.

В Draw() использовать эти сохранённые точки.

1. Платформа падает при изменении ControlPoints
Проблема: Любое изменение размера или содержимого ControlPoints (даже урезание до 2 точек) вызывает краш платформы.
Решение:

Никогда не менять ControlPoints напрямую.
Все изменения координат делать через вспомогательные переменные (_startScreen,_endScreen).

В Prepare() обновлять эти переменные, но не трогать ControlPoints.

1. OnPropertyChanged() без имени свойства
Проблема: В стабильной версии работает OnPropertyChanged(), а в новой мы использовали OnPropertyChanged(nameof(...)).
Решение: Использовать OnPropertyChanged() (без параметров) для совместимости с ObjectBase.

1. OnDragEnded() не существует
Проблема: Метод OnDragEnded() отсутствует в базовом классе ObjectBase.
Решение: Убрать вызов или заменить на OnPropertyChanged(nameof(ControlPoints)).

1. Использование System.Windows.Media
Проблема: В Tiger Trade используются свои типы цветов (XColor), а не WPF (Colors).
Решение: Убрать using System.Windows.Media; и использовать XColor.

1. Отсутствие PenWidth
Проблема: В стабильной версии есть protected override int PenWidth => LineWidth;, в новой — нет.
Решение: Добавить это свойство.

1. Prepare() не должен изменять ControlPoints
Проблема: Prepare() вызывается часто, и изменение ControlPoints в нём приводит к бесконечным циклам или крашу.

Решение:
Использовать Prepare() только для вычислений и сохранения в локальные поля.
Не трогать ControlPoints в Prepare().

✅ Что уже работает
Платформа не падает, если не трогать ControlPoints.
Prepare() и Draw() вызываются корректно.
Объект появляется в панели инструментов.
Все свойства (цвет, толщина, текст) сохраняются в шаблоны.

❌ Что не работает
Отрисовка отрезка — линия не появляется (только контрольные точки).
При попытке нарисовать объект платформа иногда падает (при изменении ControlPoints).

🚀 Следующие шаги
Реализовать отрисовку через _startScreen и_endScreen, не трогая ControlPoints.
Проверить, что Prepare() сохраняет правильные координаты.
Убедиться, что Draw() использует эти координаты.

📌 Важные файлы
src/TrendLineLibrary/Objects/HorizontalSegmentObject.cs — основной файл.
src/TrendLineLibrary/Objects/TrendLineObject.cs — стабильная версия (для сравнения).

📖 Ссылка на рабочий код
Стабильная версия TrendLineObject.cs доступна в main на GitHub.

🧠 Главные выводы
Не трогать ControlPoints — это священный массив платформы.
